...
| **675b906** | (校務PC側)HANDOVER.md (このファイル) |
| **6abe7f7** | (校務PC側)診断レポートツール `diagnostic.html` + 設定タブにリンク |
...
- 設定タブ → クラウド同期 → **📊 診断レポート** ボタン (グレー、別タブ開き) ← 6abe7f7で追加
- `recovery.html` (アプリ同ディレクトリ・独立ツール)
- `diagnostic.html` (アプリ同ディレクトリ・診断専用) ← 6abe7f7で追加
...
| # | 機能 | 状態 |
|---|------|------|
| A1 | **健全性ダッシュボード** (アプリ画面トップに3層件数の緑/黄/赤バッジ常時表示) | 未実装 |
| A2 | **整合性アラート** (件数20%減、3層乖離、queue滞留で能動警告ダイアログ) | 未実装 |
| A3 | **診断ボタン** (設定タブにリンク、3層+設定+SW状態の1画面ビュー) | ✅ **実装済 (6abe7f7)** `diagnostic.html` |
| A4 | **配布版用 自動診断** (起動時に設定壊れチェック→onboardingへ誘導) | 未実装 |
